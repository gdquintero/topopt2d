function ucKucdx = grad_mean_flex_dilation(str,xnew,p,uc,kel,gradxdil,N,nonempty,numviz,kmax)
%***********************************************************************************************************************
%ucKucdx = grad_mean_flex_dilation(str,xnew,p,uc,kel,gradxdil,N,nonempty,numviz,kmax)
%
...
for i = 1:kmax
    ...
    lin(7) = lin(8) - 1;
    ucel = uc(lin);
    ucKucdxdil(m1) = -p*(xnew(m1)^(p-1))*(ucel'*(kel*ucel)); 
end
...
for i = 1:kmax
    ...
    viz = numviz(m1);
    ind = N(1:viz,m1);
    ucKucdx(m1) = gradxdil(m1,ind)*ucKucdxdil(ind);
end