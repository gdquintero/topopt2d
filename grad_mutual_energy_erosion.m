function ubKuadx = grad_mutual_energy_erosion(str,xnew,p,ua,ub,kel,gradxer,N,nonempty,numviz,kmax)
%***********************************************************************************************************************
%ubKuadx = grad_mutual_energy_erosion(str,xnew,p,ua,ub,kel,gradxer,N,nonempty,numviz,kmax)
%
%This function computes mutual energy gradient vector for 
...
%p: Penalty parameter of SIMP model for the element densities.
...
for i = 1:kmax
    ...
    uael = ua(lin);
    ubel = ub(lin);
    ubKuadxer(m1) = -p*(xnew(m1)^(p-1))*(ubel'*(kel*uael));    
end
...
for i = 1:kmax
    ...
    viz = numviz(m1);
    ind = N(1:viz,m1);
    ubKuadx(m1) = gradxer(m1,ind)*ubKuadxer(ind);
end