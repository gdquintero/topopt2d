function gradvol = grad_volume_heaviside(str,xfil,xdens,velem,N,weigh,wi,beta,nonempty,kmax,Vmax,numviz)
%***********************************************************************************************************************
%gradvol = grad_volume_heaviside(str,xfil,velem,N,weigh,wi,beta,nonempty,kmax,Vmax,numviz)
%
...
for i = 1:kmax
    ...
    viz = numviz(m1);
    ind = N(1:viz,m1);
    betaexp = beta*exp(-beta*xdens(ind))+exp(-beta);
    gradvol(m1) = sum(((weigh(m1,1:viz)'.*velem(ind)).*(betaexp))./wi(ind))/Vmax;
end
...
%     viz = numviz(m1);
%     ind = N(1:viz,m1);
%     gradvol(m1) = sum(((weigh(m1,1:viz)'.*velem(ind)).*(betaexp(ind)))./wi(ind))/Vmax;
% end
