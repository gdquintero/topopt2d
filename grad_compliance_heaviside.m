function df0dx = grad_compliance_heaviside(str,xfil,xdens,weigh,wi,p,u,kel,gradxnew,numviz,nonempty,beta,kmax,N,lin)
%***********************************************************************************************************************
%df0dx = grad_compliance_heaviside(str,xfil,xdens,p,u,kel,gradxnew,numviz,nonempty,beta,kmax,N,lin)
%
...
n = str.nelem;
gradxdens = zeros(n,1);
betaexp = beta*exp(-beta*xdens)+exp(-beta);
% 
% %Gradient of the objective function in relation to the filtered densities:
% for i = 1:kmax
%     m1 = nonempty(i);
%     uel = u(lin(:,m1));
%     gradxdens(m1) = -p*(xfil(m1)^(p-1))*betaexp(m1)*(uel'*(kel*uel));
% end
% 
...
%     ind = numviz(m1);
%     viz = N(1:ind,m1);
%     df0dx(m1) = sum(gradxnew(m1,viz)'.*gradxdens(viz));
% end
...
for i = 1:kmax
    ...
    ind = numviz(m1);
    viz = N(1:ind,m1);
    %betaexp = beta*exp(-beta*xdens(viz))+exp(-beta);
    %df0dx(m1) = sum((gradxnew(m1,viz)'.*gradxdens(viz)).*(betaexp));
    %df0dx(m1) = sum(((weigh(m1,1:viz)'.*gradxdens(viz)))./wi(ind));
